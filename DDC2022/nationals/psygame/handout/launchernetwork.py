...
####################################################### 
# Classes for client interacting with local server
# These are classes used when talking to the challenge server.
# You shouldn't need to mess with any of these directly :shrug:
#######################################################

class LocalChannel(PsygameChannel):
    ...
    def client_send(self, user_input : str) -> dict:
        ...
        send_values = json.dumps(send_values)
        print(f"Client sends: {send_values}")
        response = self.trigger_server(send_values)
        print(f"Server response: {response}\n")
        returned_values = json.loads(response)
        return returned_values
    
    def trigger_server(self, message: str) -> str:
        move = int(json.loads(message)['move'])
        value = self.server_game.receive_input(move)
